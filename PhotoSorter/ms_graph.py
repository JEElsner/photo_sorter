...
class Graph:
    ...
    def _device_code_auth(
        ...
        if r.status_code != 200:
            raise RuntimeError(
                "Failed to initiate device code flow", r.json()["error_description"]
            )
        ...
        while True:
            ...
            if r.status_code == 400 and r.json()["error"] == "authorization_pending":
                ...
                raise RuntimeError(
                    "Failed to complete authentication", r.json()["error_description"]
                )

            # Authentication succeeded, so return the authentication token
            return r.json()

    def get_file_id(self, file_path, from_folder: str = None) -> str | None:
        header = {"Authentication": f"Bearer {self.__token}"}
        ...
        if r.status_code == 404 and r.json()["error"]["code"] == "itemNotFound":
            return None
        if r.status_code != 200:
            raise RuntimeError("Failed to get file id", r.json()["error"])

        return r.json()["id"]

    def get_file_children(self, file_id, select=None) -> iter:
        header = {"Authentication": f"Bearer {self.__token}"}
        ...
        r = requests.get(url, headers=header)
        if r.status_code != 200:
            raise RuntimeError("Failed to get file children", r.json()["error"])
        ...
        while json.get("@odata.nextLink") is not None:
            ...
            if r.status_code != 200:
                raise RuntimeError("Failed to get more child items", r.json()["error"])
    ...
    def get_file_info(self, file_id, select=None):
        ...
        r = requests.get(url, headers=header)
        if r.status_code != 200:
            raise RuntimeError("Failed to get file info", r.json()["error"])
    ...
    def move_file(self, file_id: str, new_location_id: str):
        ...
        r = requests.patch(url, headers=header, body=content)
    ...
    def create_directory(self, parent_id: str, name: str) -> str:
        ...
        r = requests.post(url, headers=header, body=body)
        if r.status_code != 200:
            raise RuntimeError(
                f"Failed to coreate new folder {name}", r.json()["error"]
            )

        return r.json()["id"]

    def ensure_path(self, base_id: str, subdirs: str) -> str:
